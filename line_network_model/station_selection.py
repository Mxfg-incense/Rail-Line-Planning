...
from line_network_model.line_model_config import (
    BBOX,
    CACHE_DIR,
...
def existing_path(primary: Path, fallback: Path | None = None) -> Path:
    ...
    raise FileNotFoundError(
        f"{primary} not found. Run: uv run python .\\line_network_model\\00_prepare_demand_data.py"
    )
...
def load_station_file() -> list[dict]:
    stations = []
    with STATION_TABLE_FILE.open("r", encoding="utf-8-sig", newline="") as f:
        for props in csv.DictReader(f):
            required = str(props.get("required", "")).strip().lower() in {"1", "true", "yes"}
            stations.append(
                {
                    ...
                    "required": required,
                    "covered_pop": float(props.get("population_value", 0.0) or 0.0),
                    "covered_jobs": float(props.get("job_value", 0.0) or 0.0),
                    "covered_commercial": float(props.get("commercial_value", 0.0) or 0.0),
                    "covered_tourism": 0.0,
                    "score": float(props.get("total_value", 0.0) or 0.0),
                    "population_value": float(props.get("population_value", 0.0) or 0.0),
                    "job_value": float(props.get("job_value", 0.0) or 0.0),
                    "transport_value": float(props.get("transport_value", 0.0) or 0.0),
                    "commercial_value": float(props.get("commercial_value", 0.0) or 0.0),
                    "total_value": float(props.get("total_value", 0.0) or 0.0),
                    "layers": props.get("layers", ""),
                    "sources": props.get("sources", ""),
                    "component_count": int(props.get("component_count", 1) or 1),
                    "name": props.get("name", ""),
                }
            )
    return stations
