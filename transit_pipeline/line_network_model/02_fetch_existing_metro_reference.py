...
from line_model_config import BBOX, CACHE_DIR, OUTPUT_DIR
from station_selection import load_roads, road_style


RAW_FILE = CACHE_DIR / f"existing_metro_{BBOX[0]}_{BBOX[1]}_{BBOX[2]}_{BBOX[3]}.json"
LINE_GEOJSON_FILE = OUTPUT_DIR / "00_existing_metro_lines.geojson"
STATION_GEOJSON_FILE = OUTPUT_DIR / "00_existing_metro_stations.geojson"
...
def write_outputs(line_features: list[dict], station_features: list[dict]) -> None:
    write_json(LINE_GEOJSON_FILE, {"type": "FeatureCollection", "features": line_features})
    write_json(STATION_GEOJSON_FILE, {"type": "FeatureCollection", "features": station_features})
...
def main() -> None:
    ...
    write_outputs(line_features, station_features)
    ...
    print(f"  line geojson: {LINE_GEOJSON_FILE}")
    print(f"  station geojson: {STATION_GEOJSON_FILE}")
